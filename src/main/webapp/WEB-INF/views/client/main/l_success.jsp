...
if (isSuccess) {
	...
	String lei_id = (String) session.getAttribute("leisure_id");//레저아이디
	int PEOPLE_NUM = Integer.parseInt((String) session.getAttribute("lPEOPLE_NUM"));//티켓수
	...
	try {
		...
		dbConnection = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "c##YGJG", "1234");

		// SQL 쿼리 실행
		String sql = "INSERT INTO payment_records_leisure (RECORD_ID, ORDERNAME, METHOD, TOTALAMOUNT, leisure_id, people_num) VALUES (PAYMENT_RECORDS_SEQUENCE.NEXTVAL, ?, ?, ?, ?, ?)";
		preparedStatement = dbConnection.prepareStatement(sql);
		preparedStatement.setString(1, orderName);//회원아이디
		preparedStatement.setString(2, method);//결제방식
		preparedStatement.setLong(3, totalAmount);//결제금액
		preparedStatement.setString(4, lei_id);//결제금액
		preparedStatement.setLong(5, PEOPLE_NUM);//결제금액
