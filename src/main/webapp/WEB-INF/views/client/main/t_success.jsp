 ...
 // 결제 승인 API 호출하기 
  ...
  if (isSuccess) {
        ...
        try {
            ...
            // SQL 쿼리 실행
            String sql = "INSERT INTO payment_records (RECORD_ID, ORDERNAME, METHOD, TOTALAMOUNT) VALUES (PAYMENT_RECORDS_SEQUENCE.NEXTVAL, ?, ?, ?)";
...
<section>
    ...
    %>
        <h1>결제 성공</h1>
        <p><span class="label">상품:</span> <%= jsonObject.get("orderName") %></p>
        <p><span class="label">결제방식:</span> <%= jsonObject.get("method") %></p>
        <p><span class="label">결제 완료 금액:</span> <%= jsonObject.get("totalAmount") %>원</p>
        <!-- 결제 일시가 API 응답에 포함되어 있다면 아래와 같이 표시할 수 있습니다. -->
        <!-- <p><span class="label">결제 일시:</span> <%= jsonObject.get("paymentDate") %></p> -->
        <p>
            <% 
            ...
            %>
            <span class="label">추가 정보:</span> <%= additionalInfo %>
        </p>
    <%} else { %>
        <h1>결제 실패</h1>
